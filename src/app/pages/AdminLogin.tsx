...
export function AdminLogin() {
  ...
  const inp = "w-full pl-12 pr-4 py-3 border border-border rounded-xl text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all bg-card text-foreground";
  ...
  return (
    // Intentional dark "security" backdrop regardless of theme — this is a
    // deliberate admin-portal aesthetic, not a neutral page background, so
    // it stays as literal dark grays/green rather than theme tokens.
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
        <div className="bg-card rounded-3xl shadow-2xl p-8 md:p-10">
          ...
          <div className="flex items-center justify-center mb-6">
            {/* Intentional gray "admin" accent, distinct from brand green */}
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 2 && (
              <motion.form key="step2" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 16 }} transition={{ duration: 0.2 }} onSubmit={handleOTP} className="space-y-4" noValidate>
                <div className="p-3 rounded-xl bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/40 text-xs text-blue-700 dark:text-blue-300">
                ...
                <div>
                  <label className="block text-sm font-medium text-foreground/80 mb-2">6-digit code</label>
                  <div className="relative">
                ...
                <button type="button" onClick={() => { setStep(1); setOtp(""); setError(null); }} className="w-full text-sm text-muted-foreground hover:text-foreground py-1 transition-colors">
          ...
          <div className="mt-6 pt-6 border-t border-border text-center">
            <Link to="/login" className="text-sm text-primary hover:underline">← Back to User Login</Link>
          </div>
        </div>
      </motion.div>
    </div>
